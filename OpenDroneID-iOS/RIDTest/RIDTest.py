...
bytes_str = input("输入数据: ")
while bytes_str:
    index = bytes_str.lower().find('fa0bbc0d')
    size_bytes = bytes.fromhex(bytes_str[index + 14:index+16])
    size = int.from_bytes(size_bytes, byteorder='little')
    odid_bytes_str = bytes_str[index + 16:]
    print("odid hex ", odid_bytes_str)
    odid_bytes = bytes.fromhex(odid_bytes_str)
    print_message_pack(odid_bytes, size)
    bytes_str = input("输入数据: ")
